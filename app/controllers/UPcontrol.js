function register() {
    const data = {
      ...
      description: '', // Se inicializa vacío
      image: '' // Se inicializa vacío
    };
  ...
  function updateProfile() {
    ...
    let xhr = new XMLHttpRequest();
    xhr.open('PUT', '/user/profile', true);
    xhr.setRequestHeader('Content-Type', 'application/json');
    ...
    xhr.onload = function() {
      if (xhr.status === 200) {
        ...
        alert("Error al actualizar perfil: " + xhr.responseText);
      }
    };
  }
  ...
  function loadUserProfile() {
    let xhr = new XMLHttpRequest();
    xhr.open('GET', '/user/profile', true); // Asegúrate de que este endpoint devuelva los datos del usuario
    xhr.setRequestHeader('Authorization', `Bearer ${sessionStorage.getItem('token')}`);
    xhr.onload = function() {
        if (xhr.status === 200) {
            ...
            console.error('Failed to fetch profile:', xhr.responseText);
        }
    };
    xhr.send();
  }
